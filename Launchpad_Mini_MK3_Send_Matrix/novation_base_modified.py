...
class NovationBase(IdentifiableControlSurface):
    ...
    def __init__(self, *a, **k):
        super(NovationBase, self).__init__(
            product_id_bytes=(
                sysex.NOVATION_MANUFACTURER_ID
                + self.model_family_code
    ...
    def _create_session_recording(self):
        self._session_recording = self.session_recording_class(
            self._target_track,
    ...
    def _create_track_recording(self):
        self._track_recording = self.track_recording_class(
            self._target_track,
