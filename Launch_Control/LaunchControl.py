#Embedded file name: /Users/versonator/Jenkins/live/output/Live/mac_64_static/Release/python-bundle/MIDI Remote Scripts/Launch_Control/LaunchControl.py
...
class LaunchControl(ControlSurface):
    ...
    def _init_mixer(self):
        ...
        bottom_encoders, top_encoders = make_all_encoders(u'Mixer', make_encoder)
        ...
        modes_layer = Layer(pan_volume_button=make_button(114, u'Pan_Volume_Mode_Button'), sends_button=make_button(115, u'Sends_Mode_Button'))
        ...
        for index in range(8):
            ...
            mute_button = make_button(pad_identifiers[index], u'Track_Mute_Button_' + str(index), is_pad=True)
    ...
    def _init_session(self):
        ...
        bottom_encoders, top_encoders = make_all_encoders(u'Session_Mixer', make_encoder)
        pan_volume_layer = Layer(volume_controls=ButtonMatrixElement(rows=[bottom_encoders]), pan_controls=ButtonMatrixElement(rows=[top_encoders]))
        self._session_mixer = SpecialMixerComponent(8, Layer(), pan_volume_layer, Layer())
        self._session_mixer.set_enabled(False)
        self._session_mixer.name = u'Session_Mixer'
        clip_launch_buttons = [ make_button(identifier, u'Clip_Launch_Button_' + str(i), is_pad=True) for i, identifier in enumerate(pad_identifiers) ]
        self._session = SpecialSessionComponent(num_tracks=8, num_scenes=0, name=u'Session')
        self._session.set_enabled(False)
        self._session.set_mixer(self._session_mixer)
        self._session_layer = Layer(track_bank_left_button=make_button(116, u'Track_Bank_Left_Button'), track_bank_right_button=make_button(117, u'Track_Bank_Right_Button'), select_prev_button=make_button(114, u'Scene_Bank_Up_Button'), select_next_button=make_button(115, u'Scene_Bank_Down_Button'), clip_launch_buttons=ButtonMatrixElement(rows=[clip_launch_buttons]))
    ...
    def _init_modes(self):
        ...
        self._modes.add_mode(u'mixer', [partial(self._session.set_mixer, self._mixer),
         LayerMode(self._session, self._mixer_track_nav_layer),
         self._mixer,
         self._session,
         self._show_controlled_tracks_message])
        self._modes.add_mode(u'session', [partial(self._session.set_mixer, self._session_mixer),
    ...
    @subject_slot(u'selected_send_index')
    def _on_selected_send_index(self, index):
        self._show_controlled_sends_message()

    @subject_slot(u'selected_mixer_mode')
    def _on_selected_mixer_mode(self, mode):
        if mode == u'sends':
            self._show_controlled_sends_message()
        else:
            self.show_message(u'Controlling Pan and Volume')
    ...
    def _show_controlled_tracks_message(self):
        ...
        if start < end:
            self.show_message(u'Controlling Track %d to %d' % (start, end))
        else:
            self.show_message(u'Controlling Track %d' % start)

    def _show_controlled_sends_message(self):
        send_index = self._mixer.selected_send_index
        send_name1 = chr(ord(u'A') + send_index)
        if send_index + 1 < self._mixer.num_sends:
            send_name2 = chr(ord(u'A') + send_index + 1)
            self.show_message(u'Controlling Send %s and %s' % (send_name1, send_name2))
        else:
            self.show_message(u'Controlling Send %s' % send_name1)

    def handle_sysex(self, midi_bytes):
        super(LaunchControl, self).handle_sysex(midi_bytes)
        if self._is_user_mode_message(midi_bytes):
            self._modes.selected_mode = u'user'
