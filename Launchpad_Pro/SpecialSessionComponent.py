        ...
        if self.is_enabled():
            if not (value is not 0 or button.is_momentary()):
                ...
                for track in self.tracks_to_use():
                    if in_range(scene_index, 0, len(track.clip_slots)):
                        if not track.playing_slot_index == scene_index:
                            if track.fired_slot_index == scene_index:
                                pass
...
, tracks):
                                    ...
                                    if find_if(lambda x: x.fired_slot_index == -2 and x.playing_slot_index == scene_index
, tracks):
                                        value_to_send = self._stop_clip_triggered_value
                                    else:
                                        value_to_send = 'Session.StoppedClip'
