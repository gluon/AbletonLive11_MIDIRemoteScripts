...
# Embedded file name: output/Live/mac_64_static/Release/python-bundle/MIDI Remote Scripts/pushbase/note_settings_component.py
...
class NoteSettingsComponentBase(Component):
    ...
    def _update_encoders(self):
        if self.is_enabled() and self._encoders:
            ...
            list(map(lambda setting: setting.encoder.set_control_element(None)
, self._settings))
...
class NoteEditorSettingsComponent(ModesComponent):
    ...
    @listenable_property
    def is_touched(self):
        return any(map(lambda e: e and e.is_touched
...
>>>>>>> d4a7b269eef325b60d6e8b8cc6298fd52c04fa34
    ...
    def _active_note_regions(self):
        all_active_regions = list(map(lambda e: e.active_note_regions
, self._editors))
...
<<<<<<< HEAD
            all_min_max_attributes = [_f for _f in map(lambda e: e.get_min_max_note_values()
, self._editors) if _f]
=======
