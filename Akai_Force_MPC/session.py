...
def find_first_playing_grouped_track(group_track, all_tracks):
    ...
    return find_if(is_playing_and_grouped, all_tracks[index_if(lambda t: t == group_track
, all_tracks):])
...
class SessionComponent(SessionComponentBase):
    ...
    def insert_scene_button(self, _):
        try:
            song = self.song
            scenes = song.scenes
            song.create_scene(clamp(index_if(lambda s: s == song.view.selected_scene
, scenes) + 1, 0, len(scenes)))
        ...
        if track_index < len(tracks):
            ...
            if self._can_have_playing_slots(track):
                if liveobj_valid(track.group_track):
                    group_track_index = index_if(lambda t: t == track.group_track
, tracks)
